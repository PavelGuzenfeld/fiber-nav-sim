...
TEST_CASE("buildProfile: terrain_z = baro_alt - agl") {
    ...
    auto profile = buildProfile(samples);
...
TEST_CASE("NCC: identical profiles produce 1.0") {
    ...
    float ncc = normalizedCrossCorrelation(a, b);
...
TEST_CASE("NCC: offset-invariant (adding constant doesn't change result)") {
    ...
    float ncc_orig = normalizedCrossCorrelation(a, b);
    float ncc_offset = normalizedCrossCorrelation(a, b_offset);
...
TEST_CASE("NCC: flat profiles return 0") {
    ...
    float ncc = normalizedCrossCorrelation(flat, ramp);
    CHECK(ncc == doctest::Approx(0.f).epsilon(1e-5));
}

TEST_CASE("NCC: empty profiles return 0") {
    std::vector<float> empty;
    float ncc = normalizedCrossCorrelation(empty, empty);
    CHECK(ncc == doctest::Approx(0.f));
}

TEST_CASE("extractDemProfile: flat DEM gives constant profile") {
    TerrainMap map;
    map.width = 50;
    map.height = 50;
    ...
    auto profile = extractDemProfile(map, 0.f, 0.f, samples);
    REQUIRE(profile.size() == 3);
...
TEST_CASE("extractDemProfile: ramp DEM gives linear profile") {
...
TEST_CASE("TerrainMap::heightAt: center of map returns center elevation") {
    auto map = makeRampDem(101, 10.f, 0.1f);

    // Center is at (0, 0) — ramp = 0.1 * 0 = 0
    float h = map.heightAt(0.f, 0.f);
    CHECK(h == doctest::Approx(0.f).epsilon(0.5f));
}

TEST_CASE("TerrainMap::heightAt: positive X gives positive elevation on ramp") {
    auto map = makeRampDem(101, 10.f, 0.1f);

    float h = map.heightAt(100.f, 0.f);
    CHECK(h == doctest::Approx(10.f).epsilon(1.0f));
}

TEST_CASE("tercomMatch: finds correct position on sine DEM") {
    ...
    for (int i = 0; i < 25; ++i) {
        ...
        float sy = true_y;
        float terrain_z = map.heightAt(sx, sy);
    ...
    auto result = tercomMatch(map, samples, true_x, true_y, config);
...
TEST_CASE("tercomMatch: finds correct position with offset search center") {
    auto map = makeSineDem(201, 12.f);
    ...
    auto result = tercomMatch(map, samples, true_x + 96.f, true_y, config);
...
TEST_CASE("tercomMatch: flat terrain gives invalid result (low PAR)") {
    TerrainMap map;
    map.width = 101;
    map.height = 101;
    ...
    auto result = tercomMatch(map, samples, 0.f, 0.f, config);

    // Flat terrain: NCC is 0 for all candidates (no variance)
    CHECK_FALSE(result.valid);
}

TEST_CASE("tercomMatch: too few samples gives invalid result") {
