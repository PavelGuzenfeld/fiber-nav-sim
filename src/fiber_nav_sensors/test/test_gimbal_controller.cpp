...
TEST_CASE("GimbalTargets.BelowThreshold.ReturnsZero")
{
    GravityVector g{0.1f, 0.5f, -0.8f};  // gx < 0.3
    auto t = gimbalTargets(g, 0.3f, 1.f, 0.7f, 1.f, 0.7f);
    CHECK(t.yaw == 0.f);
    CHECK(t.pitch == 0.f);
}

TEST_CASE("GimbalTargets.AboveThreshold.CorrectYaw")
{
...
TEST_CASE("GimbalTargets.AboveThreshold.CorrectPitch")
{
...
{
    GravityVector g{1.0f, 0.2f, 0.0f};
    auto t1 = gimbalTargets(g, 0.3f, 1.f, 0.7f, 1.f, 0.7f);
    auto t2 = gimbalTargets(g, 0.3f, 2.f, 0.7f, 1.f, 0.7f);  // 2x yaw gain
...
TEST_CASE("GimbalTargets.MaxAngleClamping")
{
    ...
    auto t = gimbalTargets(g, 0.3f, 1.f, 0.3f, 1.f, 0.3f);
    CHECK(std::abs(t.yaw) <= 0.3f + 0.001f);
    CHECK(std::abs(t.pitch) <= 0.3f + 0.001f);
}

TEST_CASE("GimbalTargets.AtThreshold.ReturnsZero")
{
    // gx exactly at threshold → should return zero (uses <=)
    GravityVector g{0.3f, 0.5f, 0.5f};
    auto t = gimbalTargets(g, 0.3f, 1.f, 0.7f, 1.f, 0.7f);
    CHECK(t.yaw == 0.f);
    CHECK(t.pitch == 0.f);
}

TEST_CASE("GimbalTargets.JustAboveThreshold")
{
    // gx barely above threshold → should produce small nonzero targets
    GravityVector g{0.31f, 0.1f, 0.1f};
    auto t = gimbalTargets(g, 0.3f, 1.f, 0.7f, 1.f, 0.7f);
    CHECK(t.yaw != 0.f);
    CHECK(t.pitch != 0.f);
}
