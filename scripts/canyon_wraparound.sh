#!/bin/bash
# Canyon wrap-around script - teleports quadtailsitter back to start when reaching end
...
echo "Monitoring quadtailsitter position..."

while true; do
    # Get current X position
    POS=$(gz topic -e -t /model/quadtailsitter/odometry -n 1 2>/dev/null | grep -A1 "x:" | head -1 | awk '{print $2}')
    ...
    if [ -n "$POS" ]; then
        ...
        if [ "$POS_INT" -gt "$CANYON_LENGTH" ] 2>/dev/null; then
            ...
            gz service -s /world/canyon_world/set_pose \
                ...
                --req "name: \"quadtailsitter\", position: {x: ${RESET_X}, y: 0, z: ${RESET_Z}}" \
